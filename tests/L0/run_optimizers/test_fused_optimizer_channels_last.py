...
class TestFusedOptimizerChannelsLast(unittest.TestCase):
    ...
    def gen_param_optim(self, tensors, options, device, tst_options=None):
        ...
        for tensor in tensors:
            #############
            input = tensor.contiguous(memory_format=torch.channels_last).to(device) # channels_last #1
            ref_input = tensor.detach().clone().contiguous().to(device)             # contiguous #1
    ...
    def gen_grad(self, ref_param, tst_param):
        for p_ref, p_tst in zip(ref_param, tst_param):
            ...
            p_ref.grad = p_tst.grad.detach().clone().contiguous()

            self.assertTrue(p_tst.grad.is_contiguous(memory_format=torch.channels_last))
    ...
    def gen_single_type_test(self, param_type=torch.float, device='cuda', *, skip_assert: bool = False):
        ...
        tensor = torch.rand([124,52,64,3], dtype=param_type, device=device)
        ref_param, tst_param, ref_optim, tst_optim = \
...
# pytest tests/L0/run_optimizers/test_fused_optimizer_channels_last.py::TestFusedSGDChannelLast -v
