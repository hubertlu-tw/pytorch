...
/*
if(c10::utils::check_env("VEC_SIZE8")) {
  constexpr int vec_size = 8;
} else {
...
void launch_vectorized_layer_norm_kernel(
  ...
  T_ACC* rstd_data
    ...
    // TODO: threads.y = 1?
    //
    dim3 threads(warp_size, num_threads() / warp_size, 1);
    if(c10::utils::check_env("FWD_LAYER_NORM_OPTIM")) { 
      threads.y = 1;
    }
    const dim3 blocks(M);
    TORCH_INTERNAL_ASSERT_DEBUG_ONLY(threads.y % 2 == 0 || threads.y == 1);
    int nshared = threads.y > 1 ? threads.y * 3/2 *sizeof(T_ACC) : 0;
    if(c10::utils::check_env("PRINT_PARAM")) {
      printf("====== threads.x=%d, threads.y=%d, nshared=%d ======\n", threads.x, threads.y, nshared);
    }
